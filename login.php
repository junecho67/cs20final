<?php 
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['username']) && isset($_GET['password'])){
        ...
        if($userExists->num_rows > 0){
            //enter users info in databse
            $checkPassword = "SELECT password FROM Users WHERE username='" . $username . "'";
            $checkPasswordResult = $conn->query($checkPassword);
            $databasePassword = $checkPasswordResult->fetch_assoc();
            if($databasePassword['password'] != $password){
                $_SESSION['wrongPassword'] = "This password you entered isn't correct.";
            }
        }
        else{
            $_SESSION['directToSignup'] = "This user doesn't exist. Please <a href='signup.php'>signup</a>.";
        }
...
<head>
  ...
  <title>Login - PawfectMatch</title>
...
<body>
  ...
  <nav class="navbar">
    ...
    <div class="nav-right">
      ...
      <a href="login.php" class="highlighted">Log In</a>
  ...
  <div class="content-container">
    <h1>Login Page</h1>
    <form onsubmit='return validate()' id="login" action="" method="get">
      ...
      <button type="submit" class="survey-btn">Log In</button>
      <p>Don't have an account? Sign up <a href="signup.php"here</a></p>
    </form>

    <?php 
      if(isset($_SESSION['directToSignup'])){
        echo '<p class="error-message">' . $_SESSION['directToSignup'] . "</p>";
        unset($_SESSION['directToSignup']);
      }
      if(isset($_SESSION['wrongPassword'])){
        echo '<p class="error-message">' . $_SESSION['wrongPassword'] . "</p>";
